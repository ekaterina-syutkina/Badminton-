                    ...
                    <tbody>
                        <?php
                            ...
                            while($row=mysql_fetch_array($result))
                            {
								$change = "";
								if($row['Change']=='-')
									$change = "<i class=\"glyphicon glyphicon-triangle-bottom\" style = \"color: green\"></i>";
								if($row['Change']=='+')
									$change = "<i class=\"glyphicon glyphicon-triangle-top\" style = \"color: red\"></i>";
								$link = 'name='.urlencode($row['Name']).'&year='.$row['BirthYear'].'&qual='.urlencode($row['Qualification']).'&cat=jo';
								echo '<tr><td>'.$row['Number'].'</td><td><a href=/dev/info.php?'.$link.'>'.$row['Name'].'</a></td><td>'.$row['BirthYear'].'</td><td>'.$row['Qualification'].'</td><td>'.$row['Region'].'</td><td>'.$row['Rating'].' '.$change.'</td><td>'.$row['Personal'].'</td><td>'.$row['Team'].'</td>
                            </tr>
                            ';// выводим данные
                            }
                    ?>
